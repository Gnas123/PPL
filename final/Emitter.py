...
# Add this __str__ method
    # def __str__(self):
    #     # We need __str__ methods for mtype and value as well for this to be fully readable
    #     mtype_str = str(self.mtype) # Relies on the __str__ of the type object (e.g., MType, IntType)
    #     value_str = str(self.value) if self.value is not None else "None" # Relies on __str__ of Index/CName or shows "None"

    #     return f"Symbol(name='{self.name}', mtype={mtype_str}, value={value_str})"
...
class Emitter():
    ...
    def getJVMType(self, inType):
        ...
        if typeIn is BoolType:
            ...
            num_ngoac_vuong = "[" * len(inType.dimens)
            if type(inType.eleType) is ArrayType:
                rettype = self.getJVMType(inType.eleType.eleType) 
            else:
                rettype = self.getJVMType(inType.eleType)
            return num_ngoac_vuong + rettype
    ...
    *    @param typ the type of the constant
    ...
    def emitALOAD(self, in_, frame):
        ...
        elif type(in_) is cgen.ArrayType or type(in_) in [cgen.ClassType, Id] or type(in_) is StringType:
    ...
    *   @param toLabel the ending label  of the scope where the variable is active.
    ...
    def emitREADVAR(self, name, inType, index, frame):
        ...
        elif type(inType) is cgen.ArrayType or type(inType) in [cgen.ClassType, Id] or type(inType) is StringType:
    ...
    *   @param in the type descriptor of the method.
    ...
    def emitINVOKESPECIAL(self, frame, lexeme=None, in_=None):
        ...
        # # !testing
        # elif not lexeme is None and in_ is None:
        #     frame.pop()
    ...
    * @param in the type descriptor of the method.
    ...
    def emitINVOKEINTERFACE(self, lexeme, in_, num, frame):
        ...
        # def emitINVOKEINTERFACE2(lexeme, typ, num):
        #     #lexeme: String
        #     #typ: String
        #     END = "\n"
        #     INDENT = "\t"
        #     return INDENT + "invokeinterface " + lexeme + typ + " " + str(num) + END
        ...
        # return emitINVOKEINTERFACE2(lexeme, self.getJVMType(in_), num)
    ...
    *   @param isStatic <code>true</code> if the method is static; <code>false</code> otherwise.
    ...
    def emitENDMETHODINTERFACE(self, frame):
        #frame: Frame
    ...
    *   @param in the type of the returned expression.
    ...
    def emitRETURN(self, in_, frame):
        ...
        elif type(in_) in [StringType, cgen.ArrayType, cgen.ClassType, Id]:
            frame.pop()
    ...
    *   @param in the code to be printed out
    ...
    def emitNEWARRAY(self, in_, frame):
        ...
        if type(in_) is IntType:
            ...
            val = "boolean"
        elif type(in_) in [StringType, Id]:
    ...
    def emitANEWARRAY(self, in_, frame):
        ...
        elif type(in_) in [cgen.ClassType, Id]:
            val = in_.name
