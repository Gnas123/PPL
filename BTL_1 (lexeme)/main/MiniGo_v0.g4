...
// # sai
// [1]int{1+1}         # expression
// [3]int{[3]int[1]}   # call array khac
...
// (endcode | EOF)
function_declared: FUNC (LP ID ID RP)? (a_func) type_withID? LB (inside_part_func)? RB endcode;
...
// Add(x, y int) int; 
sub_inside_part_interface: ID LP (func_para_list)? RP type_withID? endcode;
...
for_statement: FOR (for_basic | for_init_cond | for_range) LB (inside_part_func)? RB;
for_basic: expression;
for_init_cond: (for_assign | declared_init) SEMICOLON (expression) SEMICOLON for_assign;
