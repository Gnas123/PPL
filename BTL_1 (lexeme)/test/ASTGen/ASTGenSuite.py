...
"""
        expect = Program([
            VariablesDecl(Id("a"), ClassType(Id("Votien")), FloatLiteral(1.0))
        ])
        self.assertTrue(TestAST.test(input, str(expect), inspect.stack()[0].function))
...
"""
        expect = Program([
            StructDecl(Id("Votien"),[VariablesDecl(Id("a"), IntType(), None),VariablesDecl(Id("b"), BooleanType(), None),VariablesDecl(Id("c"), ArrayType(ClassType(Id("Votien")), [2]), None)])
        ])
        self.assertTrue(TestAST.test(input, str(expect), inspect.stack()[0].function))
    ...
    def test_079(self):
        input = """
            ...
            type b interface {} 
            func foo(){}
            func  (Cat c) foo() [2]int {}
"""
        expect = Program([
            VariablesDecl(Id("a"), None, IntLiteral(1)),
            ConstDecl(Id("b"),IntLiteral(2)),
            StructDecl(Id("a"),[]),
            InterfaceDecl(Id("b"),[]),
            FunctionDecl(Id("foo"), VoidType(), None,[],[
                ]),
            FunctionDecl(Id("foo"), ArrayType(IntType(), [2]), VariablesDecl(Id("Cat"), ClassType(Id("c")), None),[],[
                ])
        ])
        self.assertTrue(TestAST.test(input, str(expect), inspect.stack()[0].function))
...
"""
        expect = Program([
            FunctionDecl(Id("foo"), VoidType(), None,[],[
                If(IntLiteral(1), [], [(IntLiteral(1),[Return(IntLiteral(1)), Return(None)])], None),
                If(IntLiteral(1), [], None, [Return(IntLiteral(1)), Return(None)])])
        ])
        self.assertTrue(TestAST.test(input, str(expect), inspect.stack()[0].function))
