...
class StaticChecker(BaseVisitor,Utils):
    ...
    def checkType(self, left, right, list_type_permission = []):
        ...
        # if isinstance(left, Id):
        #     left = self.lookup(left.name, self.list_type, lambda x: x.name)

        # if isinstance(right, Id):
        #     right = self.lookup(right.name, self.list_type, lambda x: x.name)
    ...
    def visitProgram(self, ast, c ):
        ...
        # reduce(
        #     lambda acc, ele: [
        #         ([ketquault] + acc[0]) if isinstance(ketquault := self.visit(ele, acc), Symbol) else acc[0]
        #     ] + acc[1:], 
    ...
    def visitStructType(self, ast, c ):
        ...
        for ele in ast.elements:
            result = [visitElements(ele, result)] + result
        ast.elements = result
    ...
    def visitFuncDecl(self, ast, c ):
        ...
        visited_params = []
        for ele in ast.params:
            visited_params = [self.visit(ele, visited_params)] + visited_params
        self.visit(ast.body, [list(visited_params)] + c)
    ...
    def visitVarDecl(self, ast, c ):
        ...
        # elif isinstance(left, ArrayType) and right in [IntType, FloatType, StringType, BoolType, StructType]:
        #     if self.checkType(left.eleType, right, [(FloatType, IntType), (InterfaceType, StructType)]):
        #         return Symbol(ast.varName, left, None)
    ...
    def visitMethCall(self, ast, c):
        ...
        #     ketqua= self.lookup(ast.metName, receiver_type.methods, lambda x: x.fun.name)
        # else:
        #     ketqua=self.lookup(ast.metName, receiver_type.methods, lambda x: x.name)
